...
class Hashtag(models.Model):
    ...
    hashtag_author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="user_hashtag")  # 해시태그를 작성한 사용자와의 외래 키 관계

    class Meta:
        constraints = [
            UniqueConstraint(fields=['hashtag', 'hashtag_author'], name='unique_user_hashtag')  # 사용자당 해시태그는 고유해야 함을 보장하는 제약 조건
        ]
...
class Menu(models.Model):
    ...
    store = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="foods")  # 음식을 등록한 가게와의 외래 키 관계
